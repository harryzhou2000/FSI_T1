%shock tube problem fluid grid defining
% dfile = fopen('SC_2_0714_OUTER-1.INP','r');
dfile = fopen('INPS/BLK_LONG_T1.INP','r');
%% p,e get
lineA = '';
ps = nan(3,1000000);
es = nan(10,1000000);
while(~feof(dfile))
    ...
    if(numel(strfind(line,'*')) > 0)
        if(numel(strfind(line,'NODE')) > 0)
            ...
            piscornor = false(1,size(ps,2)); % piscornor identifies if the node is read
        end
        if(numel(strfind(lineA,'ELEMENT')) > 0)
            while(1)
                ...
                es(:,str2double(nums{1})) = [str2double(nums{2});str2double(nums{3});str2double(nums{4});str2double(nums{5});...
                    str2double(nums{6});str2double(nums{7});str2double(nums{8});str2double(nums{9});...
                    str2double(nums{10});str2double(nums{11})];
...
% dfile = fopen('SC_2_0714_OUTER-bound-.INP','r');
 dfile = fopen('INPS/BLK_LONG_T1_bound.INP','r');
nsets = 0;
sets = {};
hasend = 0;
while(~feof(dfile))
...
plot3(pc(1,pfinf2),pc(2,pfinf2),pc(3,pfinf2),'d');
plot3(pc(1,pfinf),pc(2,pfinf),pc(3,pfinf),'o');
...
%% from GETTM1.m
file = fopen('MeshCQT_BEAM_T1.txt','W');
