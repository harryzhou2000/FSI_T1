...
dfile = fopen('Triangle_I1-K1.INP','r');
%% p,e get
lineA = '';
ps = nan(3,1000000);
es = nan(4,1000000);
...
% dfile = fopen('SC_2_0714_OUTER-bound-.INP','r');
 dfile = fopen('Triangle_I1-bound-K1.INP','r');
nsets = 0;
sets = {};
hasend = 0;
while(~feof(dfile))
...
%% from GETTM1.m
file = fopen('MeshCSD_Tri_T1.txt','W');
...
fprintf(file,'%d\t%d\t%d\n',[pfinf-1,pfinf2-1,pfinter-1; 2002*ones(size(pfinf)),2003*ones(size(pfinf2)),66*ones(size(pfinter));...
