...
%% from GETTM1.m
file = fopen('MeshCSD_BEAM_T1.txt','W');
...
fprintf(file,'%d\t%d\t%d\n',[pfinf-1,pfinf2-1,pfinter-1; 2002*ones(size(pfinf)),2003*ones(size(pfinf2)),2001*ones(size(pfinter));...
    ones(size(pfinf)),2*ones(size(pfinf2)),3*ones(size(pfinter)) ]);% bound: pfinter is 66(surf + tosolid force), pf1 is 1(farfield)
fclose(file);