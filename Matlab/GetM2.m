...
importGeometry(model,'SC_2_0714_OUTER_F.STL');
...
%%
m1 = model.generateMesh('GeometricOrder','linear','Hmax',0.5,'Hmin',0.01);
...
%%
file = fopen('SC_2_0714_MeshCAD.txt','W');
...
fprintf(file,'%d\t%d\t%d\n',[pfinf-1,pfinter-1; ones(size(pfinf)),66*ones(size(pfinter));...
    ones(size(pfinf)),2*ones(size(pfinter)) ]);% bound: pfinter is 66(surf + tosolid force), pf1 is 1(farfield)
fclose(file);
